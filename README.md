...
- 右下にカウントダウンリングを常設: 表示中の枠の残り時間が減るほど弧が痩せ、
  切替（クリック送り含む）で満ちて数え直す。通常表示中のみ（掲示・臨時URL・1枠のみでは出さない）
...
- 右下段: 毎 period で1件ずつ拡大表示（時間・種別バッジ・団体名・進行中/終了済みピル）。
  巡回は**「現在の情報」カードを先頭に、当日の全予定を開始時刻順**。終了済みも巡回に残す
  （利用実績・利用組織が透過的なほうが利用の心理的ハードルが下がるため）。
  period 配分はカード・各予定 1、**進行中の貸切・イベントのみ連続 2**
  （進行中の部分利用は 1 — 「いま使えるか」はカードが担う）。
  カードは4状態（上から優先、通常表示時のみ）: 部分利用中=「一部利用可能」（時間帯付き・ティール）/
  予定あり=「ただいま利用可」+ 次の予定の時刻と種別 /
  全て終了・終日予定なし=「予約なし・利用可」（サブの文言で区別）。
